 ...
 * @return array
 */
function Trans($user_id, $password, $map = null)
{
    $map['userid'] = $user_id;
    $user_info = M('user')->where($map)->find(); //查找用户
    if (!$user_info) {
        return ['status' => 0, 'msg' => '您输入的账号有误'];
    } elseif ($user_info['status'] <= 0) {
        return ['status' => 0, 'msg' => '您的账号已锁定'];
    } else {
        $is_pwd = md5(md5($password) . $user_info['safety_salt']);
        if ($is_pwd == $user_info['safety_pwd']) {
        } else {
            return ['status' => 0, 'msg' => '交易密码错误'];
        }
    }
