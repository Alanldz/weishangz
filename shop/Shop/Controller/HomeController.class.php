...
class HomeController extends CommonController
{
    ...
    public function rechargess()
    {
        ...
        if (empty($data['username'])) {
            echo '<script>alert("请输入充值KRC账号"); window.history.back(-1); </script>';
    ...
    public function details()
    {
        $product_id = I("proid");
        $uid = session('userid');
        $product = M("product_detail")->where(array('id' => $product_id))->find();
...
//          if(!$jifentype) message("积分类型不能为空。");
    ...
    {
        $filed = 'id,pic,name,shop_type,price,ecological_total_assets';
        $search = array();
        $title = '全部商品';
        $shop_type = intval(I('shop_type', 0));
        $category = intval(I('category', -1));
        if ($_GET['search'] == 'product_name') {
            $filter = I('get.');
            if (isset($filter['name']) && !empty($filter['name'])) {
                $search['name'] = array('like', '%' . $filter['name'] . '%');
            }
            ...
            $search['shop_type'] = $shop_type;
            $title = Constants::getShopTypeItems($shop_type);
        }

        if ($category != -1) {
            $search['type_id'] = $category;
        }

        $where['status'] = Constants::YesNo_Yes;
        $goodsList = M('product_detail')->field($filed)->where($search)->where($where)->order('is_sort asc,ctime desc')->select();
        $this->assign('goodsList', $goodsList);
        $this->assign('title', $title);
        $this->assign('shop_type', $shop_type);
        $this->assign('search', $search);
        $this->assign('category', GoodsModel::getGoodsCategoryByShopType($shop_type));
        $this->display();
    }
