...
//        vendor('Alipay.Submit');
    ...
    public function index()
    {
        ...
        if ($order['status'] != 0) {  //订单验证
            error_alert("该订单不是待支付状态，无法进行支付。");
        }

        $orderDetail = M('order_detail')->where(['order_id' => $order_id])
            ->field('com_num,com_price,com_ecological_total_assets,com_ecological_total_assets_one,com_ecological_total_assets_two,com_flow_amount,com_flow_pass_card,com_product_integral')
            ->select();
        $orderPrice = [
            'price' => 0,
            'ecological_total_assets' => 0,
            'ecological_total_assets_one' => 0,
            'ecological_total_assets_two' => 0,
            'flow_amount' => 0,
            'flow_pass_card' => 0,
            'product_integral' => 0,
        ];
        foreach ($orderDetail as $item) {
            $orderPrice['price'] += $item['com_price'] * $item['com_num'];
            $orderPrice['ecological_total_assets'] += $item['com_ecological_total_assets'] * $item['com_num'];
            $orderPrice['ecological_total_assets_one'] += $item['com_ecological_total_assets_one'] * $item['com_num'];
            $orderPrice['ecological_total_assets_two'] += $item['com_ecological_total_assets_two'] * $item['com_num'];
            $orderPrice['flow_amount'] += $item['com_flow_amount'] * $item['com_num'];
            $orderPrice['flow_pass_card'] += $item['com_flow_pass_card'] * $item['com_num'];
            $orderPrice['product_integral'] += $item['com_product_integral'] * $item['com_num'];
        }
    ...
    public function pays()
    {
        if (IS_AJAX) {
            ...
            $this->ajaxReturn(['msg' => '支付成功', 'status' => 1]);
        } else {
            $this->ajaxReturn(['msg' => '请求方式有误', 'status' => 0]);
        }
    }
